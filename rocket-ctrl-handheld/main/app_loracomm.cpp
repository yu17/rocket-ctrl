...
void func_loracomm_listener(void *param) {
	if (!param) return;
	struct contact_list_t *contact_list=(struct contact_list_t *)param;
	...
	while (1) {
		...
		contact=func_contact_list_find(contact_list,packet.source_id);
...
void func_loracomm_main(struct contact_list_t *contact_list) {
	...
	while (runflag) {
		...
		func_contact_list_render(contact_list, contact, pos, 2);
		joy=joy_read(0);
		switch (joy) {
			case U:
...
void func_contact_list_destroy(struct contact_list_t *contact_list) {
	if (!contact_list || !contact_list->count) return;
	struct contact_t *contact=contact_list->contacts;
	while (contact->next) {
		contact=contact->next;
		free(contact->prev);
...
void* app_loracomm(const void *param) {
	...
	xTaskCreatePinnedToCore(&func_loracomm_listener,"LoRa_Listener",120000,&contact_list,10,&Task_LoRa_listener,1);
	func_loracomm_main(&contact_list);
	vTaskDelete(Task_LoRa_listener);
	func_contact_list_destroy(&contact_list);
	return NULL;
}